...
const Navbar = () => {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      submenu: [
        ...
        {
          label: language === "ID" ? "Bakumpul Komunitas" : "Bakumpul Komunitas",
  ...
  return (
    <nav className={`w-full sticky top-0 px-5 lg:px-8 xl:px-[8%] py-4 flex items-center justify-between z-50 bg-white dark:bg-gray-900 shadow-md transition-all ${isScrolled ? "rounded-full" : ""}`}>
      {" "}
      {/* Logo */}
      <Link to="/" onClick={handleNavClick}>
        <img src={isDarkMode ? logo_dark : logo} alt="Logo" className="w-10 cursor-pointer" />
      </Link>
      {/* Menu Desktop */}
      <ul className="hidden md:flex items-center gap-6 lg:gap-8 font-Outfit">
        {menuItems.map((item, index) => (
          <li key={index} className="relative">
            ...
            {item.submenu ? (
              <>
                <button onClick={() => handleDropdownToggle(item.label)} className="flex items-center gap-1" aria-expanded={activeDropdown === item.label}>
                  {item.label}
                  <img src={activeDropdown === item.label ? arrow_icon_dark : arrow_icon} alt="Dropdown" className={`w-3 transition-transform ${activeDropdown === item.label ? "rotate-180" : ""}`} />
                </button>
                {activeDropdown === item.label && (
                  <motion.ul initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute left-0 lg:right-0 mt-2 w-60 bg-white dark:bg-gray-800 shadow-md rounded-md">
                    {item.submenu.map((subItem, subIndex) => (
                      <li key={subIndex}>
                        <Link to={subItem.link} onClick={handleNavClick} className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700">
                          {subItem.label}
                        </Link>
                      </li>
                    ))}
              ...
              <Link to={item.link} onClick={handleNavClick} className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md">
                {item.label}
              </Link>
            )}
          </li>
      ...
      <div className="flex items-center gap-4">
        ...
        <button onClick={toggleMenu} className="md:hidden">
          <img src={isMenuOpen ? (isDarkMode ? close_white : close_black) : isDarkMode ? menu_white : menu_black} alt="Menu Icon" className="w-6" />
        </button>
      </div>
      {/* Menu Mobile */}
      {isMenuOpen && (
        ...
        >
          <button onClick={handleNavClick} className="absolute top-4 right-4">
            <img src={isDarkMode ? close_white : close_black} alt="Close Menu" className="w-4" />
          </button>

          {/* Daftar Menu */}
          <motion.ul initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col gap-2 text-base font-Outfit mt-5 w-full text-center">
            {menuItems.map((item, index) => (
              <li key={index} className="relative w-full">
                ...
                {item.submenu ? (
                      ...
                      <motion.ul initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="w-full bg-gray-100 dark:bg-gray-800 rounded-md overflow-hidden">
                        {item.submenu.map((subItem, subIndex) => (
                          <li key={subIndex} className="py-1">
                            <Link to={subItem.link} onClick={handleNavClick} className="block px-4 py-1 hover:text-primary hover:bg-gray-200 dark:hover:bg-gray-700 transition">
                              {subItem.label}
                            </Link>
                          </li>
                        ))}
                  ...
                  <Link to={item.link} onClick={handleNavClick} className="block px-4 py-1 hover:text-primary hover:bg-gray-200 dark:hover:bg-gray-700 transition">
                    {item.label}
                  </Link>
                )}
              </li>
